...
BookReader.prototype.setup = function (options) {
	...
	this.flipSpeed =
		typeof options.flipSpeed === 'number'
			? options.flipSpeed
			: {
					fast: 200,
					slow: 600,
	...
	if (options.getNumLeafs)
		this.book.getNumLeafs = options.getNumLeafs.bind(this);
	if (options.getPageWidth)
		this.book.getPageWidth = options.getPageWidth.bind(this);
	if (options.getPageHeight)
		this.book.getPageHeight = options.getPageHeight.bind(this);
	if (options.getPageURI) this.book.getPageURI = options.getPageURI.bind(this);
	if (options.getPageSide)
		this.book.getPageSide = options.getPageSide.bind(this);
	if (options.getPageNum) this.book.getPageNum = options.getPageNum.bind(this);
	if (options.getPageProp)
		this.book.getPageProp = options.getPageProp.bind(this);
	if (options.getSpreadIndices)
		this.book.getSpreadIndices = options.getSpreadIndices.bind(this);
	if (options.leafNumToIndex)
		this.book.leafNumToIndex = options.leafNumToIndex.bind(this);
...
BookReader.prototype.getActivePageContainerElements = function () {
	let containerEls = Object.values(
		this._modes.mode2Up.mode2UpLit.pageContainerCache
	)
		.map(pc => pc.$container[0])
		.concat(
			Object.values(this._modes.mode1Up.mode1UpLit.pageContainerCache).map(
				pc => pc.$container[0]
			)
		);
 ...
 */
BookReader.prototype.getActivePageContainerElementsForIndex = function (
	pageIndex
) {
	return [
		this._modes.mode2Up.mode2UpLit.pageContainerCache[pageIndex]
			?.$container?.[0],
		this._modes.mode1Up.mode1UpLit.pageContainerCache[pageIndex]
			?.$container?.[0],
		...(this.mode == this.constModeThumb
			? this.$(`.pagediv${pageIndex}`).toArray()
			: []),
	].filter(x => x);
};
...
BookReader.prototype.initParams = function () {
	...
	if (this.options.enableUrlPlugin) {
		...
		const hasHashURL =
			!Object.keys(urlParams).length && this.urlReadHashFragment();
			...
			if (params.search) {
				...
				if (searchTerm) {
					this.options.initialSearchTerm =
						utils.decodeURIComponentPlus(searchTerm);
				}
			}
		}
	}
...
BookReader.prototype.resizeBRcontainer = function (animate) {
	if (animate) {
		this.refs.$brContainer.animate(
			...
			this.constResizeAnimationDuration,
			'linear'
		);
	} else {
...
BookReader.prototype.nextReduce = function (
	...
	if (direction === 'in') {
		...
		const candidates = reductionFactors.filter(
			({ autofit }) => autofit == 'height' || autofit == 'width'
		);
		let choice = null;
...
BookReader.prototype._isIndexDisplayed = function (index) {
	return this.displayedIndices
		? this.displayedIndices.includes(index)
...
BookReader.prototype.jumpToIndex = function (index, pageX, pageY, noAnimate) {
	...
	if (!page.isViewable && page.unviewablesStart != page.index) {
		...
		const newIndex = alreadyInPreview
			? page.findNext({ combineConsecutiveUnviewables: true }).index
			: page.unviewablesStart;
...
BookReader.prototype.switchMode = function (
	mode,
	{ suppressFragmentChange = false, init = false, pageFound = false } = {}
) {
 ...
 */
BookReader.prototype.toggleFullscreen = async function (
 ...
 * @param { boolean } bindKeyboardControls
 */
BookReader.prototype.enterFullscreen = async function (
	bindKeyboardControls = true
) {
	...
	if (this.activeMode instanceof Mode1Up) {
		this.activeMode.mode1UpLit.scale =
			this.activeMode.mode1UpLit.computeDefaultScale(
				this.book.getPage(currentIndex)
			);
...
BookReader.prototype.exitFullScreen = async function () {
	...
	if (this.activeMode instanceof Mode1Up) {
		this.activeMode.mode1UpLit.scale =
			this.activeMode.mode1UpLit.computeDefaultScale(
				this.book.getPage(this.currentIndex())
			);
		this.activeMode.mode1UpLit.requestUpdate();
		await this.activeMode.mode1UpLit.updateComplete;
	}
...
BookReader.prototype.buildToolbarElement =
	Toolbar.prototype.buildToolbarElement;
	...
	this.$('.BRnavCntl').click(function () {
		...
		if ($brNavCntlBtmEl.hasClass('BRdn')) {
			if (self.refs.$BRtoolbar)
				promises.push(
					self.refs.$BRtoolbar
						.animate({ top: self.getToolBarHeight() * -1 })
						.promise()
				);
			promises.push(
				self
					.$('.BRfooter')
					.animate({ bottom: self.getFooterHeight() * -1 })
					.promise()
			);
...
BookReader.prototype.updateFromParams = function (params) {
	...
	const { mode = 0, init = false, fragmentChange = false } = params;
...
BookReader.prototype.getFooterHeight = function () {
	const $heightEl =
		this.mode == this.constMode2up ? this.refs.$BRfooter : this.refs.$BRnav;
