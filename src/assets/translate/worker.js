...
let translationService, responseOptions, input = undefined;
...
`;
  ...
  for (const item of uniqueVocabFiles.values()) {
  ...
  const constructedAlignedMemories = await Promise.all([prepareAlignedMemoryFromBuffer(modelBuffer, 256),
    prepareAlignedMemoryFromBuffer(shortListBuffer, 64)]);
  const alignedModelMemory = constructedAlignedMemories[0];
  const alignedShortlistMemory = constructedAlignedMemories[1];
  const alignedVocabsMemoryList = new Module.AlignedMemoryList;
  for (const item of downloadedVocabBuffers) {
    const alignedMemory = await prepareAlignedMemoryFromBuffer(item, 64);
  ...
  const translationModel = new Module.TranslationModel(modelConfig, alignedModelMemory, alignedShortlistMemory, alignedVocabsMemoryList);
...
const translate = (from, to, paragraphs) => {
  ...
  if (from !== 'en' && to !== 'en') {
    ...
    const translatedParagraphsInEnglish = translateInvolvingEnglish(from, 'en', paragraphs);
...
const translateInvolvingEnglish = (from, to, paragraphs) => {
  ...
  const result = translationService.translate(translationModel, input, responseOptions);
