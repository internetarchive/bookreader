...
export class ModeThumb {
	...
	drawLeafs(seekIndex) {
		...
		const pageViewBuffer =
			floor((this.br.refs.$brContainer.prop('scrollWidth') - maxRight) / 2) -
			14;
		...
		for (let i = 0; i < leafMap.length; i++) {
			...
			const bottomInView =
				leafBottom >= scrollTop && leafBottom <= scrollBottom;
		...
		for (const row of rowsToDisplay) {
			if (notInArray(row, this.br.displayedRows)) {
				...
				for (const { num: leaf, left: leafLeft } of leafMap[row].leafs) {
					const leafWidth = this.br.thumbWidth;
					const leafHeight = floor(
						(book.getPageHeight(leaf) * this.br.thumbWidth) /
							book.getPageWidth(leaf)
					);
					...
					const idealReduce = floor(
						book.getPageWidth(leaf) / this.br.thumbWidth
					);
					...
					pageContainer.$container.data('leaf', leaf).on('mouseup', event => {
						...
						this.br._components.navbar.updateViewModeButton(
							$('.viewmode'),
							'twopg',
							'Two-page view'
						);
		...
		this.br
			...
			.removeClass('BRpagedivthumb_highlight');

		// highlight current page
		this.br
			.$('.pagediv' + this.br.currentIndex())
			.addClass('BRpagedivthumb_highlight');
	}
	...
	getThumbnailWidth(thumbnailColumns) {
		...
		const width =
			(this.br.refs.$brPageViewEl.width() - padding) / (thumbnailColumns + 0.5); // extra 0.5 is for some space at sides
		const idealThumbnailWidth = Math.floor(width);
		return idealThumbnailWidth > 0
			? idealThumbnailWidth
	...
	prepare() {
		...
		this.dragScrollable =
			this.dragScrollable ||
			new DragScrollable(this.br.refs.$brContainer[0], {
				preventDefault: true,
			});
	...
	jumpToIndex(index) {
		...
		for (let i = 0; i <= index; i++) {
			...
			const leafHeight = floor(
				(book.getPageHeight(leafIndex) * this.br.thumbWidth) /
					book.getPageWidth(leafIndex)
			);
