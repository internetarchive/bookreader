...
export function runBaseTests (br) {
  ...
  test.disablePageCaching('Canonical URL shows parameters if cookie set', async t => {
    ...
    const goForward = await isRightToLeft() ? nav.desktop.goLeft : nav.desktop.goRight;
  ...
  test('2up mode - Clicking `Previous page` changes the page', async t => {
    ...
    const goPrevious = await isRightToLeft() ? nav.desktop.goRight : nav.desktop.goLeft;
    const goNext = await isRightToLeft() ? nav.desktop.goLeft : nav.desktop.goRight;
