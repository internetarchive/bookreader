 ...
 * @param  {Object} options
 */
  BookReader.prototype.setup = (options) => {
 ...
 */
  BookReader.util = {
    ...
    decodeURIComponentPlus(value) {
      // Decodes a URI component and converts '+' to ' '
      return decodeURIComponent(value).replace(/\+/g, ' ');
    },

    encodeURIComponentPlus(value) {
 ...
 * @private
 */
  BookReader.prototype.extendParams = (params, newParams) => {
    const modifiedNewParams = $.extend({}, newParams);
    if (typeof (modifiedNewParams.page) !== 'undefined') {
      const pageIndex = this.parsePageString(modifiedNewParams.page);
      // eslint-disable-next-line no-restricted-globals
 ...
 */
  BookReader.prototype.initParams = () => {
    const params = {};
    ...
    if (typeof (this.titleLeaf) !== 'undefined' && this.getNumLeafs() > 2) {
 ...
 */
  BookReader.prototype.getInitialMode = (params) => {
    // Use params or browser width to set view mode
    const windowWidth = $(window).width();
    let nextMode;
    if (typeof (params.mode) !== 'undefined') {
      nextMode = params.mode;
    } else if (this.ui === 'full' && this.enableMobileNav && this.isFullscreenActive && windowWidth <= this.onePageMinBreakpoint
    ) {
 ...
 */
  BookReader.prototype.init = () => {
    ...
    const initialMode = this.getInitialMode(params);
    ...
    this.refs.$brContainer.bind('scroll', this, (e) => {
    ...
    $(window).on('orientationchange', this, (e) => {
      e.data.resize();
    }).bind(this);

    if (this.protected) {
      $(document).on('contextmenu dragstart', '.BRpagediv1up', () => false);
      $(document).on('contextmenu dragstart', '.BRpageimage', () => false);
      $(document).on('contextmenu dragstart', '.BRpagedivthumb', () => false);
  ...
  BookReader.prototype.trigger = (name, props) => {
    $(document).trigger(`BookReader:${name}`, this, props);
  };

  BookReader.prototype.bind = (name, callback) => {
    $(document).bind(`BookReader:${name}`, callback);
  };

  BookReader.prototype.unbind = (name, callback) => {
    $(document).bind(`BookReader:${name}`, callback);
  };
 ...
 */
  ...
  BookReader.prototype.setupTooltips = () => {
    this.$('.js-tooltip').each((idx, el) => {
      const options = {
  ...
  BookReader.prototype.bindGestures = (jElement) => {
    // support gesture change is only iOS. Support android.
    // Momentum scrolling is causing the scroll position
    ...
    jElement.unbind('touchmove').bind('touchmove', (e) => {
      if (e.originalEvent.cancelable) numTouches = e.originalEvent.touches.length;
      e.stopPropagation();
    });

    jElement.unbind('gesturechange').bind('gesturechange', (e) => {
      e.preventDefault();
  ...
  BookReader.prototype.setClickHandler2UP = (element, data, handler) => {
    $(element).unbind('mousedown').bind('mousedown', data, (e) => {
      handler(this, e);
    });
  };

  BookReader.prototype.drawLeafsOnePage = () => {
    ...
    for (i = 0; i < this.getNumLeafs(); i++) {
      // eslint-disable-next-line no-underscore-dangle
      height = parseInt(this._getPageHeight(i) / this.reduce, 2);
    ...
    if ((firstIndexToDraw !== 0) && (this.reduce > 1)) {
      // eslint-disable-next-line no-plusplus
      firstIndexToDraw--;
    ...
    if (((this.getNumLeafs() - 1) !== lastIndexToDraw) && (this.reduce > 1)) {
    ...
    for (i = 0; i < firstIndexToDraw; i + 1) {
      // eslint-disable-next-line no-underscore-dangle
      leafTop += parseInt(this._getPageHeight(i) / this.reduce, 2) + 10;
    }

    for (i = 0; i < indicesToDisplay.length; i + 1) {
      index = indicesToDisplay[i];
      // eslint-disable-next-line no-underscore-dangle
      height = parseInt(this._getPageHeight(index) / this.reduce, 2);

      if (BookReader.util.notInArray(indicesToDisplay[i], this.displayedIndices)) {
        // eslint-disable-next-line no-underscore-dangle
        const width = parseInt(this._getPageWidth(index) / this.reduce, 2);
        // eslint-disable-next-line radix
        const leftMargin = parseInt((containerWidth - width) / 2);
    ...
    for (i = 0; i < this.displayedIndices.length; i + 1) {
 ...
 */
  BookReader.prototype.drawLeafsThumbnail = (seekIndex) => {
    ...
    for (i = 0; i < this.getNumLeafs(); i + 1) {
    ...
    const pageViewBuffer = Math.floor((this.refs.$brContainer.prop('scrollWidth') - maxRight) / 2) - 14;
    ...
    let leastVisible = this.getNumLeafs() - 1;
    let mostVisible = 0;

    // Determine the thumbnails in view
    for (i = 0; i < leafMap.length; i + 1) {
    ...
    for (i = 1; i < this.thumbRowBuffer + 1; i + 1) {
    ...
    for (i = 0; i < rowsToDisplay.length; i + 1) {
      if (BookReader.util.notInArray(rowsToDisplay[i], this.displayedRows)) {
        row = rowsToDisplay[i];

        for (j = 0; j < leafMap[row].leafs.length; j + 1) {
          index = j;
          ...
          const thumbReduce = Math.floor(this.getPageWidth(leaf) / this.thumbWidth);
    ...
    let k;
    for (i = 0; i < this.displayedRows.length; i + 1) {
      if (BookReader.util.notInArray(this.displayedRows[i], rowsToDisplay)) {
        row = this.displayedRows[i];
        for (k = 0; k < leafMap[row].leafs.length; k + 1) {
  ...
  BookReader.prototype.lazyLoadThumbnails = () => {
    ...
    if (toLoad > 0) {
      ...
      this.refs.$brPageViewEl.find('img.BRlazyload').filter(`:lt(${toLoad})`).each(() => {
        self.lazyLoadImage(this);
      });
    }
  };

  BookReader.prototype.lazyLoadImage = (dummyImage) => {
    let img = new Image();
    const self = this;
    ...
    $(img)
      ...
      .one('load', () => {
        ...
        setTimeout(() => { self.lazyLoadThumbnails(); }, 100);
      })
      .one('error', () => {
  ...
  BookReader.prototype.drawLeafsTwoPage = () => {
    const $twoPageViewEl = this.refs.$brTwoPageView;

    // $$$ we should use calculated values in this.twoPage (recalc if necessary)
    const indexL = this.twoPage.currentIndexL;
    const top = this.twoPageTop();
    ...
    const indexR = this.twoPage.currentIndexR;
 ...
 */
  BookReader.prototype.zoom = (direction) => {
    switch (this.mode) {
  ...
  BookReader.prototype.zoom1up = (direction) => {
 ...
 */
  BookReader.prototype.resizeBRcontainer = (animate) => {
    if (animate) {
 ...
 */
  BookReader.prototype.resizePageView1up = () => {
    ...
    if (oldScrollTop > 0) {
      ...
      const fudgeFactor = (this.getPageHeight(this.currentIndex()) / this.reduce) * 0.6;
    ...
    if (this.onePage.autofit) {
      const reductionFactor = this.nextReduce(this.reduce,
 ...
 */
  BookReader.prototype.onePageCalculateViewDimensions = (reduce, padding) => {
    let viewWidth = 0;
    let viewHeight = 0;
    for (let i = 0; i < this.getNumLeafs(); i + 1) {
      viewHeight += parseInt(this._getPageHeight(i) / reduce, 10) + padding;
      const width = parseInt(this._getPageWidth(i) / reduce, 10);
 ...
 */
  ...
  BookReader.prototype.zoom2up = (direction) => {
    ...
    // Get new zoom state
    const reductionFactor = this.nextReduce(this.reduce, direction, this.twoPage.reductionFactors);
    if ((this.reduce === reductionFactor.reduce)
    ...
    const oldCenter = this.twoPageGetViewCenter();

    // If zooming in, reload imgs.  DOM elements will be removed by prepareTwoPageView
    // $$$ An improvement would be to use the low res image until the larger one is loaded.

    if (direction === 1) {
      for (let i = 0; i < this.prefetchedImgs; i + 1) {
        if (this[i] !== 0) {
          delete this.prefetchedImgs[i];
        }
      }
    }
  ...
  BookReader.prototype.zoomThumb = (direction) => {
 ...
 * @param {number}
 */
  BookReader.prototype.getThumbnailWidth = (thumbnailColumns) => {
    const padding = (thumbnailColumns + 1) * this.thumbPadding;
    const width = (this.refs.$brPageViewEl.width() - padding)
     / (thumbnailColumns + 0.5); // extra 0.5 is for some space at sides
 ...
 * @return {number}
 */
  BookReader.prototype.quantizeReduce = (reduce, reductionFactors) => {
    let quantized = reductionFactors[0].reduce;
    let distance = Math.abs(reduce - quantized);
    let newDistance;

    for (let i = 1; i < reductionFactors.length; i + 1) {
 ...
 */
  BookReader.prototype.nextReduce = (currentReduce, direction, reductionFactors) => {
    ...
    if (direction === 'in') {
      newReduceIndex = 0;
      for (i = 1; i < reductionFactors.length; i + 1) {
      ...
      let choice = null;
      for (i = 0; i < reductionFactors.length; i + 1) {
 ...
 */
  BookReader.prototype.jumpToPage = (pageNum) => {
    const pageIndex = this.parsePageString(pageNum);
 ...
 */
  BookReader.prototype.jumpToIndex = (index, pageX, pageY, noAnimate) => {
    const self = this;
    ...
    if (this.constMode2up === this.mode) {
      ...
      if (pageY) {
        offset = parseInt((pageY) / this.reduce, 10);
      ...
      if (pageX) {
        offset = parseInt((pageX) / this.reduce, 10);
        // eslint-disable-next-line no-bitwise
 ...
 * @param {number}
 */
  BookReader.prototype.switchMode = (mode, options) => {
    if (mode === this.mode) {
      return;
    }
  ...
  BookReader.prototype.updateBrClasses = () => {
    const modeToClass = {};
  ...
  BookReader.prototype.isFullscreen = () => this.isFullscreenActive;

  BookReader.prototype.toggleFullscreen = () => {
    if (this.isFullscreen()) {
      this.exitFullScreen();
    } else {
  ...
  BookReader.prototype.enterFullscreen = () => {
  ...
  BookReader.prototype.exitFullScreen = () => {
    this.refs.$brContainer.css('opacity', 0);
 ...
 */
  BookReader.prototype.prepareOnePageView = () => {
  ...
  BookReader.prototype.prepareThumbnailView = () => {
    this.refs.$brContainer.empty();
    this.refs.$brContainer.css({
      overflowY: 'scroll',
      overflowX: 'auto',
 ...
 * @param {number}
 */
  BookReader.prototype.prepareTwoPageView = (centerPercentageX, centerPercentageY) => {
    ...
    let targetLeaf = this.firstIndex;
 ...
 */
  BookReader.prototype.prepareTwoPagePopUp = () => {
    ...
    $(this.leafEdgeL).add(this.leafEdgeR).bind('mouseenter', this, (e) => {
      $(e.data.twoPagePopUp).show();
    });

    $(this.leafEdgeL).add(this.leafEdgeR).bind('mouseleave', this, (e) => {
      $(e.data.twoPagePopUp).hide();
    });

    $(this.leafEdgeL).bind('click', this, (e) => {
      e.data.trigger(BookReader.eventNames.stop);
      const jumpIndex = e.data.jumpIndexForLeftEdgePageX(e.pageX);
      e.data.jumpToIndex(jumpIndex);
    });

    $(this.leafEdgeR).bind('click', this, (e) => {
      e.data.trigger(BookReader.eventNames.stop);
      const jumpIndex = e.data.jumpIndexForRightEdgePageX(e.pageX);
      e.data.jumpToIndex(jumpIndex);
    });

    $(this.leafEdgeR).bind('mousemove', this, (e) => {
      const jumpIndex = e.data.jumpIndexForRightEdgePageX(e.pageX);
    ...
    $(this.leafEdgeL).bind('mousemove', this, (e) => {
      const jumpIndex = e.data.jumpIndexForLeftEdgePageX(e.pageX);
      $(e.data.twoPagePopUp).text(`View ${e.data.getPageName(BookReader.util.clamp(jumpIndex, 0, e.data.getNumLeafs() - 1))}`);
 ...
 */
  BookReader.prototype.calculateSpreadSize = () => {
    const firstIndex = this.twoPage.currentIndexL;
    const secondIndex = this.twoPage.currentIndexR;
    ...
    const leftWidthFromCenter = this.twoPage.scaledWL - leftGutterOffset + this.twoPage.leafEdgeWidthL;
    const rightWidthFromCenter = this.twoPage.scaledWR + leftGutterOffset + this.twoPage.leafEdgeWidthR;
  ...
  BookReader.prototype.getIdealSpreadSize = (firstIndex, secondIndex) => {
    const ideal = {};
    ...
    const totalLeafEdgeWidth = parseInt(this.getNumLeafs() * 0.1, 10);
    const maxLeafEdgeWidth = parseInt(this.refs.$brContainer.prop('clientWidth') * 0.1, 10);
    ideal.totalLeafEdgeWidth = Math.min(totalLeafEdgeWidth, maxLeafEdgeWidth);

    const widthOutsidePages = 2 * (this.twoPage.coverInternalPadding + this.twoPage.coverExternalPadding) + ideal.totalLeafEdgeWidth;
    const heightOutsidePages = 2 * (this.twoPage.coverInternalPadding + this.twoPage.coverExternalPadding);
    ...
    if (ideal.height / ratio <= ideal.width) {
      // use height
      ideal.width = parseInt(ideal.height / ratio, 10);
    } else {
      // use width
      ideal.height = parseInt(ideal.width * ratio, 10);
    }
 ...
 * @return {Object}
 */
  BookReader.prototype.getSpreadSizeFromReduce = (firstIndex, secondIndex, reduce) => {
    const spreadSize = {};
    // $$$ Scale this based on reduce?
    const totalLeafEdgeWidth = parseInt(this.getNumLeafs() * 0.1, 10);
    const maxLeafEdgeWidth = parseInt(this.refs.$brContainer.prop('clientWidth') * 0.1, 10); // $$$ Assumes leaf edge width constant at all zoom levels
    spreadSize.totalLeafEdgeWidth = Math.min(totalLeafEdgeWidth, maxLeafEdgeWidth);

    // $$$ Possibly incorrect -- we should make height "dominant"
    const nativeWidth = this._getPageWidth(firstIndex) + this._getPageWidth(secondIndex);
    const nativeHeight = this._getPageHeight(firstIndex) + this._getPageHeight(secondIndex);
    spreadSize.height = parseInt((nativeHeight / 2) / this.reduce, 10);
    spreadSize.width = parseInt((nativeWidth / 2) / this.reduce, 10);
 ...
 * @return {number}
 */
  BookReader.prototype.twoPageGetAutofitReduce = () => {
    const spreadSize = this.getIdealSpreadSize(this.twoPage.currentIndexL, this.twoPage.currentIndexR);
 ...
 * @return {boolean}
 */
  BookReader.prototype.twoPageIsZoomedIn = () => {
 ...
 * @return {number}
 */
  BookReader.prototype.onePageGetPageTop = (index) => {
    let i;
    let leafTop = 0;
    let h;
    for (i = 0; i < index; i + 1) {
      h = parseInt(this._getPageHeight(i) / this.reduce, 10);
      leafTop += h + this.padding;
    }
    return leafTop;
  };

  BookReader.prototype.getMedianPageSize = () => {
    ...
    this._medianPageSize = { width: widths[parseInt(widths.length / 2, 10)], height: heights[parseInt(heights.length / 2, 10)] };
    return this._medianPageSize;
  };
 ...
 */
  BookReader.prototype.onePageCalculateReductionFactors = () => {
  ...
  BookReader.prototype.twoPageCalculateReductionFactors = () => {
 ...
 */
  BookReader.prototype.twoPageSetCursor = () => {
    const $twoPageViewEl = this.refs.$brTwoPageView;
    if (($twoPageViewEl.width() > this.refs.$brContainer.prop('clientWidth'))
      ...
      if (this.prefetchedImgs[this.twoPage.currentIndexL]) this.prefetchedImgs[this.twoPage.currentIndexL].style.cursor = 'move';
      if (this.prefetchedImgs[this.twoPage.currentIndexR]) this.prefetchedImgs[this.twoPage.currentIndexR].style.cursor = 'move';
    } else {
      if (this.prefetchedImgs[this.twoPage.currentIndexL]) this.prefetchedImgs[this.twoPage.currentIndexL].style.cursor = '';
      if (this.prefetchedImgs[this.twoPage.currentIndexR]) this.prefetchedImgs[this.twoPage.currentIndexR].style.cursor = '';
    }
  };
 ...
 * @throws
 */
  BookReader.prototype.currentIndex = () => {
    // $$$ we should be cleaner with our idea of which index is active in 1up/2up
    if (this.mode === this.constMode1up || this.mode === this.constModeThumb) {
      return this.firstIndex; // $$$ page in center of view would be better
 ...
 * @param {number}
 */
  BookReader.prototype.updateFirstIndex = (index, options) => {
 ...
 */
  BookReader.prototype.rightmost = () => {
    if (this.pageProgression !== 'rl') {
      this.last();
    } else {
      this.first();
 ...
 */
  ...
  BookReader.prototype.next = () => {
    if (this.constMode2up === this.mode) {
      this.trigger(BookReader.eventNames.stop);
      this.flipFwdToIndex(null);
    } else if (this.firstIndex < this.lastDisplayableIndex()) {
      this.jumpToIndex(this.firstIndex + 1);
    }
  };

  BookReader.prototype.prev = () => {
    if (this.constMode2up === this.mode) {
      this.trigger(BookReader.eventNames.stop);
  ...
  BookReader.prototype.first = () => {
    this.jumpToIndex(this.firstDisplayableIndex());
  };

  BookReader.prototype.last = () => {
 ...
 */
  BookReader.prototype._scrollAmount = () => {
    if (this.constMode1up === this.mode) {
      // Overlap by % of page size
      // eslint-disable-next-line no-mixed-operators
      return parseInt(this.refs.$brContainer.prop('clientHeight') - this.getPageHeight(this.currentIndex()) / this.reduce * 0.03, 10);
    }

    return parseInt(0.9 * this.refs.$brContainer.prop('clientHeight'), 10);
  };

  /**
 * @param {Number|null} index to flip back one spread, pass index=null
 */
  BookReader.prototype.flipBackToIndex = (index) => {
    if (this.constMode1up === this.mode) return;

    const leftIndex = this.twoPage.currentIndexL;

    if (this.animating) return;

    if (this.leafEdgeTmp !== null) {
      // eslint-disable-next-line no-alert
    ...
    const previousIndices = this.getSpreadIndices(index);
 ...
 * @param {number}
 */
  BookReader.prototype.flipLeftToRight = (newIndexL, newIndexR) => {
    const leftLeaf = this.twoPage.currentIndexL;

    const oldLeafEdgeWidthL = this.leafEdgeWidth(this.twoPage.currentIndexL);
    const newLeafEdgeWidthL = this.leafEdgeWidth(newIndexL);
    const leafEdgeTmpW = oldLeafEdgeWidthL - newLeafEdgeWidthL;

    const currWidthL = this.getPageWidth2UP(leftLeaf);
    const newWidthL = this.getPageWidth2UP(newIndexL);
    const newWidthR = this.getPageWidth2UP(newIndexR);

    const top = this.twoPageTop();
    const gutter = this.twoPage.middle + this.gutterOffsetForIndex(newIndexL);
    ...
    const { left } = $(this.prefetchedImgs[leftLeaf]).offset();
    // $$$ This seems very similar to the gutter.  May be able to consolidate the logic.
    const right = `${$twoPageViewEl.prop('clientWidth') - left - $(this.prefetchedImgs[leftLeaf]).width() + $twoPageViewEl.offset().left - 2}px`;
    ...
    $(this.prefetchedImgs[leftLeaf]).animate({ width: '0px' }, self.flipSpeed, 'easeInSine', () => {
      $(self.leafEdgeTmp).animate({ left: `${gutter + newWidthR}px` }, self.flipSpeed, 'easeOutSine');

      self.$('.BRgutter').css({ left: `${gutter - self.twoPage.bookSpineDivWidth * 0.5}px` });

      $(self.prefetchedImgs[newIndexR]).animate({ width: `${newWidthR}px` }, self.flipSpeed, 'easeOutSine', () => {
        ...
        // $$$ refactor with opposite direction flip
 ...
 * @param {number}
 */
  BookReader.prototype.flipFwdToIndex = (index) => {
    if (this.animating) return;
    ...
    const nextIndices = this.getSpreadIndices(index);
    let gutter;
 ...
 * @param {number}
 */
  BookReader.prototype.flipRightToLeft = (newIndexL, newIndexR) => {
    const oldLeafEdgeWidthL = this.leafEdgeWidth(this.twoPage.currentIndexL);
    const oldLeafEdgeWidthR = this.twoPage.edgeWidth - oldLeafEdgeWidthL;
    const newLeafEdgeWidthL = this.leafEdgeWidth(newIndexL);
    const newLeafEdgeWidthR = this.twoPage.edgeWidth - newLeafEdgeWidthL;

    const leafEdgeTmpW = oldLeafEdgeWidthR - newLeafEdgeWidthR;

    const top = this.twoPageTop();
    const scaledW = this.getPageWidth2UP(this.twoPage.currentIndexR);

    const { middle } = this.twoPage;
    ...
    const speed = this.flipSpeed;

    if (this.enableSearch) this.removeSearchHilites();

    $(this.leafEdgeTmp).animate({ left: gutter }, speed, 'easeInSine');
    $(this.prefetchedImgs[this.twoPage.currentIndexR]).animate({ width: '0px' }, speed, 'easeInSine', () => {
      self.$('BRgutter').css({ left: `${gutter - self.twoPage.bookSpineDivWidth * 0.5}px` });
      $(self.leafEdgeTmp).animate({ left: `${gutter - newWidthL - leafEdgeTmpW}px` }, speed, 'easeOutSine');
      $(self.prefetchedImgs[newIndexL]).animate({ width: `${newWidthL}px` }, speed, 'easeOutSine', () => {
  ...
  BookReader.prototype.prefetchImg = (index) => {
    const pageURI = this._getPageURI(index);
 ...
 * @param {number}
 */
  BookReader.prototype.prepareFlipLeftToRight = (prevL, prevR) => {
    this.prefetchImg(prevL);
    this.prefetchImg(prevR);

    const height = this._getPageHeight(prevL);
    const width = this._getPageWidth(prevL);
    const { middle } = this.twoPage.middle;
    const top = this.twoPageTop();
    // eslint-disable-next-line no-mixed-operators
    const scaledW = this.twoPage.height * width / height; // $$$ assumes height of page is dominant
  ...
  BookReader.prototype.prepareFlipRightToLeft = (nextL, nextR) => {
    // Prefetch images
    this.prefetchImg(nextL);
    this.prefetchImg(nextR);

    let height = this._getPageHeight(nextR);
    let width = this._getPageWidth(nextR);
    const { middle } = this.twoPage.middle;
    const top = this.twoPageTop();
    let scaledW = (this.twoPage.height * width) / height;
  ...
  BookReader.prototype.pruneUnusedImgs = () => {
    ...
    for (const key in this.prefetchedImgs) {
  ...
  BookReader.prototype.prefetch = () => {
    ...
    const adjacentPagesToLoad = 3;

    const lowCurrent = Math.min(this.twoPage.currentIndexL, this.twoPage.currentIndexR);
    const highCurrent = Math.max(this.twoPage.currentIndexL, this.twoPage.currentIndexR);

    const start = Math.max(lowCurrent - adjacentPagesToLoad, 0);
    const end = Math.min(highCurrent + adjacentPagesToLoad, this.getNumLeafs() - 1);

    // Load images spreading out from current
    for (let i = 1; i <= adjacentPagesToLoad; i + 1) {
      const goingDown = lowCurrent - i;
      if (goingDown >= start) {
        this.prefetchImg(goingDown);
      }
      const goingUp = highCurrent + i;
      if (goingUp <= end) {
  ...
  BookReader.prototype.getPageWidth2UP = (index) => {
    // We return the width based on the dominant height
    const height = this._getPageHeight(index);
    const width = this._getPageWidth(index);
 ...
 */
  BookReader.prototype.twoPageGutter = () => this.twoPage.middle + this.gutterOffsetForIndex(this.twoPage.currentIndexL);

  /**
 * Returns the offset for the top of the page images
 */
  BookReader.prototype.twoPageTop = () => this.twoPage.coverExternalPadding + this.twoPage.coverInternalPadding;
 ...
 * @return {number}
 */
  BookReader.prototype.twoPageCoverWidth = (totalPageWidth) => totalPageWidth + this.twoPage.edgeWidth + 2 * this.twoPage.coverInternalPadding;

  /**
 * Returns the percentage offset into twopageview div at the center of container div
 * { percentageX: float, percentageY: float }
 */
  BookReader.prototype.twoPageGetViewCenter = () => {
    const center = {};

    const containerOffset = this.refs.$brContainer.offset();
    const viewOffset = this.refs.$brTwoPageView.offset();
    // eslint-disable-next-line no-bitwise
 ...
 * @param {number}
 */
  BookReader.prototype.twoPageCenterView = (percentageX, percentageY) => {
    if (typeof (percentageX) === 'undefined') {
      percentageX = 0.5;
    }
    if (typeof (percentageY) === 'undefined') {
      percentageY = 0.5;
    }

    const viewWidth = this.refs.$brTwoPageView.width();
    const containerClientWidth = this.refs.$brContainer.prop('clientWidth');
    const intoViewX = percentageX * viewWidth;

    const viewHeight = this.refs.$brTwoPageView.height();
    const containerClientHeight = this.refs.$brContainer.prop('clientHeight');
 ...
 * @return {number}
 */
  BookReader.prototype.twoPageFlipAreaHeight = () => parseInt(this.twoPage.height, 10);

  /**
 * Returns the the integer width of the flip areas
 * @return {number}
 */
  BookReader.prototype.twoPageFlipAreaWidth = () => {
    const max = 100; // $$$ TODO base on view width?
    const min = 10;

    const width = this.twoPage.width * 0.15;
    return parseInt(BookReader.util.clamp(width, min, max), 10);
  };
 ...
 * @return {number}
 */
  BookReader.prototype.twoPageFlipAreaTop = () => parseInt(this.twoPage.bookCoverDivTop + this.twoPage.coverInternalPadding, 10);

  /**
 * Left offset for left flip area
 * @return {number}
 */
  BookReader.prototype.twoPageLeftFlipAreaLeft = () => parseInt(this.twoPage.gutter - this.twoPage.scaledWL, 10);

  /**
 * Left offset for right flip area
 * @return {number}
 */
  BookReader.prototype.twoPageRightFlipAreaLeft = () => parseInt(this.twoPage.gutter + this.twoPage.scaledWR - this.twoPageFlipAreaWidth(), 10);

  /**
 * Position calculation shared between search and text-to-speech functions
 */
  BookReader.prototype.setHilightCss2UP = (div, index, left, right, top, bottom) => {
    // We calculate the reduction factor for the specific page because it can be different
    // for each page in the spread
    const height = this._getPageHeight(index);
    const width = this._getPageWidth(index);
    const reduce = this.twoPage.height / height;
    const scaledW = parseInt(width * reduce, 10);

    const gutter = this.twoPageGutter();
    let pageL;
 ...
 */
  BookReader.prototype.stopFlipAnimations = () => {
    this.trigger(BookReader.eventNames.stop);
    ...
    // eslint-disable-next-line no-undef
    jQuery.each(this.prefetchedImgs, () => {
 ...
 * @return {boolean}
 */
  BookReader.prototype.keyboardNavigationIsDisabled = (event) => event.target.tagName === 'INPUT';
 ...
 * @return {number}
 */
  BookReader.prototype.gutterOffsetForIndex = (pindex) => {
    // To find the offset of the gutter from the middle we calculate our percentage distance
    // through the book (0..1), remap to (-0.5..0.5) and multiply by the total page edge width
    let offset = parseInt(((pindex / this.getNumLeafs()) - 0.5) * this.twoPage.edgeWidth, 10);
 ...
 * @return {number}
 */
  BookReader.prototype.leafEdgeWidth = (pindex) => {
    // $$$ could there be single pixel rounding errors for L vs R?
    if ((this.getPageSide(pindex) === 'L') && (this.pageProgression !== 'rl')) {
      return parseInt((pindex / this.getNumLeafs()) * this.twoPage.edgeWidth + 0.5, 10);
    }
    return parseInt((1 - pindex / this.getNumLeafs()) * this.twoPage.edgeWidth + 0.5, 10);
  };
 ...
 * @return {number}
 */
  BookReader.prototype.jumpIndexForLeftEdgePageX = (pageX) => {
    let jumpIndex;
    if (this.pageProgression !== 'rl') {
 ...
 */
  BookReader.prototype.jumpIndexForRightEdgePageX = (pageX) => {
    let jumpIndex;
    if (this.pageProgression !== 'rl') {
      // LTR
 ...
 */
  BookReader.prototype.initEmbedNavbar = () => {
    // IA-specific
 ...
 * @return {string}
 */
  BookReader.prototype.getNavPageNumString = (index) => {
 ...
 */
  BookReader.prototype.getNavPageNumHtml = (index, numLeafs, pageNum, pageType, maxPageNum) => {
    let pageStr;
    if (pageNum[0] !== 'n') {
 ...
 * @param {number}
 */
  BookReader.prototype.updateNavPageNum = (index) => {
 ...
 * @param {number}
 */
  BookReader.prototype.updateNavIndex = (index) => {
 ...
 */
  ...
  BookReader.prototype.initToolbar = (mode, ui) => {
    const self = this;
  ...
  BookReader.prototype.blankInfoDiv = () => $([
  ...
  BookReader.prototype.blankShareDiv = () => $([
 ...
 * @param {number} reduce
 */
  BookReader.prototype.updateToolbarZoom = (reduce) => {
    let value;
    let autofit = null;

    // preserve zoom/fit for each mode
    if (this.mode === this.constMode2up) {
 ...
 */
  BookReader.prototype.bindNavigationHandlers = () => {
    ...
    jIcons.filter('.onepg').bind('click', () => {
      self.switchMode(self.constMode1up);
    });

    jIcons.filter('.twopg').bind('click', () => {
      self.switchMode(self.constMode2up);
    });

    jIcons.filter('.thumb').bind('click', () => {
      self.switchMode(self.constModeThumb);
    });

    jIcons.filter('.fit').bind('fit', () => {
      // XXXmang implement autofit zoom
    });

    jIcons.filter('.book_left').click(() => {
      self.trigger(BookReader.eventNames.stop);
      self.left();
      return false;
    });

    jIcons.filter('.book_right').click(() => {
      self.trigger(BookReader.eventNames.stop);
      self.right();
      return false;
    });

    jIcons.filter('.book_up').bind('click', () => {
    ...
    jIcons.filter('.book_down').bind('click', () => {
      if ($.inArray(self.mode, [self.constMode1up, self.constModeThumb]) >= 0) {
        self.scrollDown();
      } else {
        self.next();
    ...
    jIcons.filter('.book_top').click(() => {
      self.first();
      return false;
    });

    jIcons.filter('.book_bottom').click(() => {
      self.last();
      return false;
    });

    jIcons.filter('.book_leftmost').click(() => {
      self.leftmost();
      return false;
    });

    jIcons.filter('.book_rightmost').click(() => {
      self.rightmost();
      return false;
    });

    jIcons.filter('.zoom_in').bind('click', () => {
      self.trigger(BookReader.eventNames.stop);
      self.zoom(1);
    ...
    jIcons.filter('.zoom_out').bind('click', () => {
      self.trigger(BookReader.eventNames.stop);
      self.zoom(-1);
    ...
    jIcons.filter('.full').bind('click', () => {
      if (self.ui === 'embed') {
        const url = self.$('.BRembedreturn a').attr('href');
        window.open(url);
      } else {
 ...
 */
  BookReader.prototype.navigationMousemoveHandler = (event) => {
  ...
  BookReader.prototype.initSwipeData = (clientX, clientY) => {
    /*
  ...
  BookReader.prototype.swipeMousedownHandler = (event) => {
    ...
    $(event.target).bind('mouseout.swipe',
      { br: self },
      self.swipeMouseupHandler).bind('mouseup.swipe',
      { br: self },
      self.swipeMouseupHandler).bind('mousemove.swipe',
      { br: self },
  ...
  BookReader.prototype.swipeMousemoveHandler = (event) => {
    const self = event.data.br;
    const { _swipe } = self;
  ...
  BookReader.prototype.swipeMouseupHandler = (event) => {
    const { _swipe } = event.data.br;
  ...
  BookReader.prototype.bindMozTouchHandlers = () => {
    const self = this;

    // Currently only want touch handlers in 2up
    this.refs.$br.bind('MozTouchDown', (event) => {
      if (this.mode === self.constMode2up) {
        event.preventDefault();
      }
    })
      .bind('MozTouchMove', (event) => {
        if (this.mode === self.constMode2up) {
          event.preventDefault();
        }
      })
      .bind('MozTouchUp', (event) => {
        if (this.mode === self.constMode2up) {
          event.preventDefault();
        }
      });
 ...
 * @return {boolean}
 */
  BookReader.prototype.navigationIsVisible = () => {
 ...
 */
  BookReader.prototype.hideNavigation = () => {
 ...
 */
  BookReader.prototype.showNavigation = () => {
 ...
 * @return {number}
 */
  BookReader.prototype.firstDisplayableIndex = () => {
    if (this.mode !== this.constMode2up) {
      return 0;
    }
 ...
 * @return {number}
 */
  BookReader.prototype.lastDisplayableIndex = () => {
    const lastIndex = this.getNumLeafs() - 1;
 ...
 * @param {Object}
 */
  BookReader.prototype.updateFromParams = (params) => {
    if (typeof (params.mode) !== 'undefined') {
    ...
    // process /search
    if (this.enableSearch && typeof (params.search) !== 'undefined') {
    ...
    // $$$ process /theme
    if (this.enableThemesPlugin && typeof (params.theme) !== 'undefined') {
 ...
 */
  BookReader.prototype.getPageIndex = (pageNum) => {
 ...
 * @return {array}
 */
  BookReader.prototype.getPageIndices = (pageNum) => {
    const indices = [];
    ...
    if (pageNum.slice(0, 1) === 'n') {
      try {
        ...
        const pageIndex = parseInt(pageIntStr, 10);
 ...
 * @return {string}
 */
  BookReader.prototype.getPageName = (index) => `Page ${this.getPageNum(index)}`;
 ...
 * @return {boolean}
 */
  BookReader.prototype.canSwitchToMode = (mode) => {
 ...
 */
  BookReader.prototype._getPageWidth = (index) => {
 ...
 */
  BookReader.prototype._getPageHeight = (index) => {
    const clampedIndex = BookReader.util.clamp(index, 0, this.getNumLeafs() - 1);
 ...
 * @return {string}
 */
  BookReader.prototype._getPageURI = (index, reduce, rotate) => {
    ...
    if (typeof (reduce) === 'undefined') {
      ...
      const ratio = this.getPageHeight(index) / this.twoPage.height;
      let scale;
 ...
 * @param {string}
 */
  BookReader.prototype.showProgressPopup = (msg) => {
  ...
  BookReader.prototype.removeProgressPopup = () => {
    $(this.popup).remove();
    this.$('.BRprogresspopup').remove();
    this.popup = null;
  };

  BookReader.prototype.buildShareDiv = (jShareDiv) => {
    ...
    jForm.find('.fieldset-embed input').bind('change', () => {
    ...
    jForm.find('input, textarea').bind('focus', () => {
      this.select();
    });
    ...
    jForm.find('.facebook-share-button').click(() => {
      const params = $.param({ u: self._getSocialShareUrl() });
      const url = `https://www.facebook.com/sharer.php?${params}`;
      self.createPopup(url, 600, 400, 'Share');
    });
    jForm.find('.twitter-share-button').click(() => {
      const params = $.param({
    ...
    jForm.find('.email-share-button').click(() => {
      const body = `${self.bookTitle}\n\n${self._getSocialShareUrl()}`;
  ...
  BookReader.prototype._getSocialShareUrl = () => {
 ...
 */
  BookReader.prototype.buildInfoDiv = (jInfoDiv) => {
    ...
    const footerEl = '<div class="BRfloatFoot BRinfoFooter"></div>';
    ...
    const childrenEl = $('<div class="BRinfoW mv20-lg">').append(children);
 ...
 * Can be overriden
 */
  BookReader.prototype.initUIStrings = () => {
 ...
 */
  BookReader.prototype.createPopup = (href, width, height, name) => {
  // Fixes dual-screen position
    const dualScreenLeft = window.screenLeft !== undefined ? window.screenLeft : screen.left;
    const dualScreenTop = window.screenTop !== undefined ? window.screenTop : screen.top;

    // eslint-disable-next-line no-nested-ternary
    const winW = window.innerWidth ? window.innerWidth : document.documentElement.clientWidth ? document.documentElement.clientWidth : screen.width;
    // eslint-disable-next-line no-nested-ternary
    const winH = window.innerHeight ? window.innerHeight : document.documentElement.clientHeight ? document.documentElement.clientHeight : screen.height;

    const left = ((winW / 2) - (width / 2)) + dualScreenLeft;
    const top = ((winH / 2) - (height / 2)) + dualScreenTop;
    const url = href;
    const opts = `${'status=1'
               + ',width='}${width
    },height=${height
 ...
 */
  BookReader.prototype.reloadImages = () => {
    this.refs.$brContainer.find('img').each((index, elem) => {
 ...
 */
  BookReader.prototype.getToolBarHeight = () => {
    if (this.refs.$BRtoolbar && this.refs.$BRtoolbar.css('display') === 'block') {
      return (this.refs.$BRtoolbar.outerHeight() + parseInt(this.refs.$BRtoolbar.css('top'), 10));
    }
    return 0;
  };
 ...
 * @return {number}
 */
  BookReader.prototype.getFooterHeight = () => {
    ...
    if ($heightEl && this.refs.$BRfooter) {
      ...
      const bottom = parseInt(this.refs.$BRfooter.css('bottom'), 10);
 ...
 */
  BookReader.prototype.getNumLeafs = () => {
 ...
 */
  BookReader.prototype.getPageWidth = (index) => this.getPageProp(index, 'width');

  /**
 * @param  {number} index
 * @return {Number|undefined}
 */
  BookReader.prototype.getPageHeight = (index) => this.getPageProp(index, 'height');
 ...
 */
  // eslint-disable-next-line no-unused-vars
  BookReader.prototype.getPageURI = (index, reduce, rotate) => this.getPageProp(index, 'uri');

  /**
 * @param  {number} index
 * @return {string} - L or R
 */
  BookReader.prototype.getPageSide = (index) => {
    let pageSide = this.getPageProp(index, 'pageSide');
    if (!pageSide) {
 ...
 * @return {string}
 */
  BookReader.prototype.getPageNum = (index) => {
    let pageNum = this.getPageProp(index, 'pageNum');
 ...
 */
  BookReader.prototype.getPageProp = (index, propName) => this._getDataProp(index, propName);
 ...
 * @return {array} - eg [0, 1]
 */
  BookReader.prototype.getSpreadIndices = (pindex) => {
    let spreadIndices;
    if (this.pageProgression === 'rl') {
 ...
 * @return {number}
 */
  BookReader.prototype.leafNumToIndex = (leafNum) => {
    const dataf = this._getDataFlattened();
    for (let i = 0; i < dataf.length; i + 1) {
 ...
 * @return {Object}
 */
  BookReader.prototype.paramsFromCurrent = () => {
    const params = {};
 ...
 * @return {Object}
 */
  BookReader.prototype.paramsFromFragment = (fragment) => {
    const params = {};
    ...
    // Simple #nn syntax
    const oldStyleLeafNum = parseInt(/^\d+$/.exec(fragment), 10);
    ...
    } else if (urlHash.mode === '2up') {
      params.mode = this.constMode2up;
    } else if (urlHash.mode === 'thumb') {
 ...
 * @return {string}
 */
  BookReader.prototype.fragmentFromParams = (params) => {
    const separator = '/';
 ...
 * @return {number|undefined}
 */
  BookReader.prototype.parsePageString = (pageNum) => {
    let pageIndex;
 ...
 * @return {array}
 */
  BookReader.prototype._getDataFlattened = () => {
    if (this._getDataFlattened.cached
      && this._getDataFlattened.cached[1] === this.data.length) return this._getDataFlattened.cached[0];
 ...
 * @return {array}
 */
  BookReader.prototype._getDataProp = (index, prop) => {
    const dataf = this._getDataFlattened();
    if (isNaN(index) || index < 0 || index >= dataf.length) return;
    if (typeof (dataf[index][prop]) === 'undefined') return;
    // eslint-disable-next-line consistent-return
  ...
  */
  BookReader.prototype.$ = (selector) => this.refs.$br.find(selector);
