 ...
 */
 (function setUpUrlPlugin() {
    var MESSAGE_TYPE_PATH_CHANGE = 'bookReaderPathChange';
    ...
    function setUpIframeCommunication(bookReader) {
        ...
        bookReader.bind('pathChange', function notifyParent() {
            var path = bookReader.fragmentFromParams(
                bookReader.paramsFromCurrent()
            );

            window.parent.postMessage(
                createWindowMessage(MESSAGE_TYPE_PATH_CHANGE, { path: path }),
                '*'
            );
        });
        ...
        window.addEventListener('message', function updatePath(event) {
            ...
            if (!event.data || event.data.type !== MESSAGE_TYPE_PATH_CHANGE) {
                return;
            }

            bookReader.updateFromParams(
                bookReader.paramsFromFragment(event.data.path)
            );
        });
    }
